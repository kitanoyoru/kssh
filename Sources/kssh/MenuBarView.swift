...
private enum Route: Equatable {
    ...
    var parent: Route {
        switch self {
        case .main, .profilesList, .createGPGKey, .createSSHKey, .renameKey:
            return .main
...
struct MenuBarView: View {
    ...
    private var mainContent: some View {
        VStack(spacing: 0) {
            ...
            if viewModel.isLoading && viewModel.sshKeys.isEmpty {
                ...
                // is compact; GPG collapsed) — no scrolling.
                VStack(spacing: Spacing.sm) {
                    keysSection
                    gitSection
                    gpgSection
    ...
    /// Per-key context menu: copy, rename, delete. Extracted so the
